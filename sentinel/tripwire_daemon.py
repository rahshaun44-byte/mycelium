...
# ── Configuration ─────────────────────────────────────────────────────────────
STATE_DIR = Path.home() / ".qflex" / "tripwire"
STATE_DIR.mkdir(parents=True, exist_ok=True)
TRIPWIRE_LOG = STATE_DIR / "tripwire_chain.jsonl"
BASELINE_FILE = STATE_DIR / "baseline_hashes.json"
...
def log_boot_event(logger: TripwireLogger):
    ...
    logger.log("SYSTEM_BOOT", f"Daemon active at {uptime or datetime.now().isoformat()}", "INFO")
